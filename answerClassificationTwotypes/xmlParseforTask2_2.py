...
#解析xml文件
try:
        tree = ET.parse("data/xml/dev.xml")
        root = tree.getroot()
except Exception as e:
        print("Errot:parse file xml failed!")
        sys.exit(1)
#预处理结果输出文件地址
fp = open("data/task2/YN_dev_out_5.txt",'w+')
fr = open("dev_result_new.txt","r")
devNum = 0
dev_lines = fr.readlines()
print("*"*50)
for Question in root.findall('Question'):    
        #此部分用于提取YES_NO类型问题与答案,其他时，请注释
        if(Question.get("QTYPE") != "YES_NO"):
                devNum = devNum+len(Question.findall('Comment'))
                continue
